...
def add_single_entry_to_table(table_name, material_name, material_class, material_property, datatype, value):
    # Create a connection to the database
    engine = create_connection()


    # Check if the data fits the specified format
    if not all(isinstance(arg, str) for arg in [material_name, material_class, material_property]):
        raise ValueError("Material name, class, and property must be strings")
    ...
    required_columns = ['material_name', 'material_class', material_property]
